...
int main(int argc, char** argv) {
	...
	compile_sub.add_option("-o,--output", optimized_output_path, "Compressed webassembly output (.owasm)");
	std::string qr_path;
	...
	auto& run_sub  = *app.add_subcommand("run", "Run optimized Webassembly");
	...
	if(compile_sub) {
		...
		stop       = std::chrono::high_resolution_clock::now();
		time_taken = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
		fmt::print("Optimized wasm: {} bytes / {} bits ({}ms)\n", out_optimized.size(), size, time_taken);
		...
		if(!qr_path_run.empty()) {
			...
			stop       = std::chrono::high_resolution_clock::now();
			time_taken = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
			fmt::print("Input optimized wasm: {} bytes ({}ms)\n", optimized_wasm_bytes.size(), time_taken);
		} else if(!wasm_input_run.empty()) {
			start = std::chrono::high_resolution_clock::now();
			std::ifstream wasm_file(wasm_input_run, std::ios::binary);
			optimized_wasm_bytes = std::vector<uint8_t>((std::istreambuf_iterator<char>(wasm_file)), std::istreambuf_iterator<char>());
			stop                 = std::chrono::high_resolution_clock::now();
			time_taken           = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
			fmt::print("Input optimized wasm: {} bytes ({}ms)\n", optimized_wasm_bytes.size(), time_taken);
		}
