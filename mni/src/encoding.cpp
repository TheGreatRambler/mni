...
namespace Mni {
	namespace Encoding {
		uint64_t PrependSize(
			...
			auto size_bits = Mni::Encoding::GetRequiredLEBBits(size, DEFAULT_LEB_MULTIPLE);
			current_bit    = Mni::Encoding::MoveBits(
				   size_current_bit, current_bit, size_current_bit + size_bits, bytes);
			return Mni::Encoding::WriteLEBUnsigned(
					   size, DEFAULT_LEB_MULTIPLE, size_current_bit, bytes)
				   + size;
		}
		...
		uint64_t Write1Bit(bool bit, uint64_t current_bit, std::vector<uint8_t>& bytes) {
			...
			bytes[current_bit >> 3]
				^= (-!!bit ^ bytes[current_bit >> 3]) & (0b10000000 >> (current_bit % 8));
